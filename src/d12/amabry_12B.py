...
while q:
    ...
    if path[-1] == "end":
        ...
        visited_twice = max(path.count(cave) for cave in path if cave.islower()) == 2
        q.extend((*path, edge) for edge in caves[path[-1]] if edge.isupper() or (edge not in path) or (edge not in ("start", "end") and not visited_twice))
